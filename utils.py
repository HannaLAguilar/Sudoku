...
def img_numbers(img, numbers, color=(255, 0, 255)):
    ...
    for x in range(0, 9):
        for y in range(0, 9):
            if numbers[(y * 9) + x] != 0:
                cv2.putText(img, str(numbers[(y * 9) + x]), (x * width + int(width / 2) - 10, int((y + 0.8) * width)),
                            cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale=2, color=color, thickness=2, lineType=cv2.LINE_AA)
    return img
