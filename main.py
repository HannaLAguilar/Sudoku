...
# Sudoku digits
numbers = split_box_numbers(img_warp_gray)
sudoku = get_numbers(numbers, Classifier(), 'classifier_digit.pt')
sudoku_unsolving = sudoku.copy()

# Sudoku solve
sudoku_solver.solve(sudoku)
sudoku_numbers = [ii for ii in sudoku_unsolving.ravel()]
sudoku_solve_numbers = [ii for ii in (sudoku-sudoku_unsolving).ravel()]


# Visualization
img_stack = ([img, img_threshold, img_contours, img_points],
             [img_warp_gray, img_numbers(np.zeros_like(img), sudoku_numbers, color=(0, 255, 0)),
              img_numbers(np.zeros_like(img), sudoku_solve_numbers),
              img_numbers(img_warp,  sudoku_solve_numbers)])
